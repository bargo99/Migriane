...
#change age from character to number
Migraine<- Migraine%>%
  mutate(age = case_when(
    age == "19 years old" ~ 19,
    age== "٢٠ سنة"  ~20,age== "22 سنة"~22,
    TRUE ~ as.double(age)
  ))
#rename observations
Migraine<-Migraine %>%
  mutate(accept=recode(accept,  نعم="yes",لا="no"),
         ...
         relieve=recode(relieve,"النوم أو الراحة"="rest","التدليك"="massage","الأدوية المسكنة"="analgesics","أخرى"="others","الأدوية المسكنة, أخرى"="analgesics,others","التدليك, الأدوية المسكنة "="massage,analgesics","النوم أو الراحة, أخرى"="rest,others","النوم أو الراحة, الأدوية المسكنة" ="rest,analgesics","النوم أو الراحة, الأدوية المسكنة, أخرى" ="rest,analgesics,others","النوم أو الراحة, التدليك"="rest,massage","النوم أو الراحة, التدليك, الأدوية المسكنة"="rest,massage,analgesics",
                        "النوم أو الراحة, التدليك, الأدوية المسكنة, أخرى"="rest,massage,analgesics,others","التدليك, الأدوية المسكنة"="massage,analgesics","غثيان, تقيؤ, رهاب الصوت (حساسية للصوت), انزعاج في البطن"="nausea,vomiting,phonophobia,GI distress"),
...
#change ID migraine questionaire from character to factor
Migraine<-Migraine %>% 
...
#creating table and plot
#install.packages("gtsummary")
library(gtsummary)
#delete raw randomly tell sample size= 284 (1,2,3,4,5,134,97,32,143)
Migraine<-Migraine[-c(1,2,3,4,5,134,97,32,143,200),]
#filtered data frame which only contain positive value
positive_cases <- Migraine %>%
  filter(migraine_test=="positive"&!is.na(migraine_test))
#saving the file
saveRDS(positive_cases, file = "positive_cases.rds")
saveRDS(Migraine, file = "migraine.rds")
